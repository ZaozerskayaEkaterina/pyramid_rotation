#5 Правильная пирамида / Изометрия / Не параллельна координатным осям
...
while True:
    ...
    ax.set_ylim([-3, 3])
    ax.set_zlim([0, 5])
    ax.view_init(45, 45)  # Установка углов поворота осей

    # Рисуем рёбра пирамиды
    ax.plot3D(rotated_vertices[[0, 1], 1], rotated_vertices[[0, 1], 0], rotated_vertices[[0, 1], 2], 'green')
    ax.plot3D(rotated_vertices[[0, 2], 1], rotated_vertices[[0, 2], 0], rotated_vertices[[0, 2], 2], 'green')
    ax.plot3D(rotated_vertices[[0, 3], 1], rotated_vertices[[0, 3], 0], rotated_vertices[[0, 3], 2], 'green')
    ax.plot3D(rotated_vertices[[0, 4], 1], rotated_vertices[[0, 4], 0], rotated_vertices[[0, 4], 2], 'green')

    # Рисуем основание пирамиды
    ax.plot3D(rotated_vertices[1:5, 1], rotated_vertices[1:5, 0], rotated_vertices[1:5, 2], 'green')
    ax.plot3D([rotated_vertices[1, 1], rotated_vertices[4, 1]], [rotated_vertices[1, 0], rotated_vertices[4, 0]],
...
plt.close(fig)  # Закрываем фигуру после завершения анимации